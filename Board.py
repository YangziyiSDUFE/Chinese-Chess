...
class chess_board:
    def __init__(self):
        ...
        for i in range(9):
            self.board.append([])
            for j in range(10):
                if (const.init_borad[i][j] == 0):
                    self.board[i].append(chess(-1, const.init_borad[i][j]))
                else:
                    self.board[i].append(chess(0 if j < 5 else 1, const.init_borad[i][j]))
    def copy(self):
        new_board = chess_board()
        for i in range(9):
            for j in range(10):
                new_board.board[i][j].belong = self.board[i][j].belong
                new_board.board[i][j].chess_type = self.board[i][j].chess_type
        return new_board
